...
export function uApp (app:TemplatedApp) : Wrap {
    ...
    const wrapInterface = {
        useExpress(path: string, middleware: any){
            app.any(path, (res:HttpResponse, req: HttpRequest) => {
                middleware(req, res, () => {});
            })
            return this;
        },
        use(fnOrPath:ShadowMiddleware | string, router?: RequestMap) {
            ...
            if(typeof fnOrPath === 'string' && (router as RequestMap).isRequestMap ){
                const route: URouter = router!!._route;
                Object.keys(route).forEach(httpMethod => {
                    // @ts-ignore
                    Object.keys(route[httpMethod as TemplatedAppMethods]).forEach(pathname => {
                        const path = (fnOrPath.slice(-1)[0] === '/') ? fnOrPath.slice(0, -1) : fnOrPath;
                        const absolutePath = String(path + pathname);
                        
                        // @ts-ignore
                        const func = route[httpMethod as TemplatedAppMethods][pathname];
